/**
 * anchor-cloud.js — AnchorBrowser cloud example for NextGen EHR
 *
...
const { login_nextgen, schedule_patient } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting NextGen EHR automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_nextgen(page, {});
      log('Action 1 result:', data1);

      const data2 = await schedule_patient(page, {});
