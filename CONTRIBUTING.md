# Contributing to NextGen EHR Browser Automation
...
- **Broken selectors**: NextGen EHR updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real NextGen EHR instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.NEXTGEN_URL}/path/to/section`);
...
When you find working selectors for NextGen EHR, document them:

```javascript
// NextGen EHR selectors (verified YYYY-MM-DD)
...
NextGen EHR updates its UI periodically. When selectors break:
